...
@interface DeviceInfoVC () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, SmartDeviceDelegate>
{
...
- (void)viewDidLoad {
    ...
    [self.tableData addObject:@[@"Device model"]];
    [self.tableData addObject:@[@"Date of production", @"Device UUID", @"Battery voltage"]];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([title containsString:@"Device model"]) {
        switch (self.smartDevice.baseInfo.product_info.type)
        {
            case SmartDeviceProductTypeBattery:
                cell.detailTextLabel.text = @"SmartBatteryPack";
                break;
    ...
    } else if ([title containsString:@"Date of production"]) {
        NSDateFormatter *dateFormatter = [NSDateFormatter new];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"]; // 格式化时间
        cell.detailTextLabel.text = [dateFormatter stringFromDate:self.smartDevice.baseInfo.dateOfProduction];
    } else if ([title containsString:@"Device UUID"]) {
        cell.detailTextLabel.text = self.smartDevice.baseInfo.uuid;
    } else if ([title containsString:@"Battery voltage"]) {
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%0.1fv", self.smartDevice.battery.totalVoltage];
    }
